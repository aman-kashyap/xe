...
// Provider is the implementation of `g.Provider` for accessing Gitlab.
...
func userFromReader(r io.Reader, user *g.User) error {
	u := struct {
...
func (p *Provider) RefreshToken(refreshToken string) (*oauth2.Token, error) {
	...
	ts := p.config.TokenSource(g.ContextForClient(p.Client()), token)
