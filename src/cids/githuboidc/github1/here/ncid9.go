...
func cmd() *cobra.Command {
	...
	c.Flags().StringVar(&a.clientID, "client-id", "6c0971e3c5016f020e0f", "OAuth2 client ID of this application.")
	c.Flags().StringVar(&a.clientSecret, "client-secret", "5dd8069b23820aaad1afb6c215f6eb5cd72f33c2", "OAuth2 client secret of this application.")
	c.Flags().StringVar(&a.redirectURI, "redirect-uri", "http://localhost:8888/dex/callback", "Callback URL for OAuth2 responses.")
	c.Flags().StringVar(&issuerURL, "issuer", "http://localhost:6788", "URL of the OpenID Connect issuer.")
	c.Flags().StringVar(&listen, "listen", "http://localhost:8888", "HTTP(S) address to listen at.")
	c.Flags().StringVar(&tlsCert, "tls-cert", "key.pem", "X509 cert file to present when serving HTTPS.")
	c.Flags().StringVar(&tlsKey, "tls-key", "key.pem", "Private key for the HTTPS cert.")
	c.Flags().StringVar(&rootCAs, "issuer-root-ca", "cert.pem", "Root certificate authorities for the issuer. Defaults to host certs.")
